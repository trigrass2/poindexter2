...
Slave::Slave(CyclicCommunicationManager::Pointer manager, uint16_t index) : manager(manager)
{
	// Read the parameters out for now
	// Might aswell use the statics for this :P
	type = SlaveType(manager, index);
	revision = SlaveRev(manager, index);
	build = SlaveBuild(manager, index);
	numFMMU = readBytePositional(manager, index, SLAVE_SUPPORTED_FMMU);
	numSyncManager = readBytePositional(manager, index, SLAVE_SUPPORTED_SM);
	...
	std::cerr << "Slave" 
		          ...
		          << " PDI Control " << (int)readBytePositional(manager, index, SLAVE_PDI_CONTROL)
		          << " ESC Features " << std::hex << readShortPositional(manager, index, SLAVE_ESC_FEATURES) << std::dec
		          << std::endl;

	// Remap
	writeShortPositional(manager, index, SLAVE_STATION_ADDR, newSlaveAddr);
	...
	try
	{
		uint16_t testAddr = readShortConfigured(manager, slaveAddr, SLAVE_STATION_ADDR);
	}
...
void Slave::ReadEEPROM(uint32_t address, uint8_t* data, int count)
{
	while(count)
	{
		...
		do
		{
			status = readShortConfigured(this->manager, this->slaveAddr, SLAVE_EEPROM_CONTROL);
			status &= SLAVE_EEPROM_CONTROL_COMMAND_MASK;
		} while(status != SLAVE_EEPROM_CONTROL_COMMAND_IDLE);

		// Write out the address
		writeWordConfigured(this->manager, this->slaveAddr, SLAVE_EEPROM_ADDRESS, address & 0xFFFF);

		// Then the go command
		status = readShortConfigured(this->manager, this->slaveAddr, SLAVE_EEPROM_CONTROL);
		status |= SLAVE_EEPROM_CONTROL_COMMAND_READ;
		writeShortConfigured(this->manager, this->slaveAddr, SLAVE_EEPROM_CONTROL, status);
		...
		// Get the data
		uint16_t read_data = readShortConfigured(this->manager, this->slaveAddr, SLAVE_EEPROM_DATA);
...
int Slave::NumSlaves(CyclicCommunicationManager::Pointer manager)
{
	...
	CyclicCommunicationManager::CyclicFuture f = manager->SendDatagram(datagram);
	f.get();
...
// The static helpers.
uint8_t Slave::SlaveType(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx)
{
	return readBytePositional(manager, slaveIdx, SLAVE_TYPE);
}

uint8_t Slave::SlaveRev(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx)
{
	return readBytePositional(manager, slaveIdx, SLAVE_REVISION);;
}

uint16_t Slave::SlaveBuild(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx)
{
	return readShortPositional(manager, slaveIdx, SLAVE_BUILD);
}

// Private helpers
uint8_t Slave::readByteConfigured(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address)
{
...
uint16_t Slave::readShortConfigured(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address)
{
...
uint32_t Slave::readWordConfigured(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address)
{
...
uint8_t Slave::readBytePositional(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address)
{
...
uint16_t Slave::readShortPositional(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address)
{
...
uint32_t Slave::readWordPositional(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address)
{
...
void Slave::writeBytePositional(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address, uint8_t data)
{
...
void Slave::writeShortPositional(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address, uint16_t data)
{
...
void Slave::writeWordPositional(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address, uint32_t data)
{
...
void Slave::writeByteConfigured(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address, uint8_t data)
{
...
void Slave::writeShortConfigured(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address, uint16_t data)
{
...
void Slave::writeWordConfigured(CyclicCommunicationManager::Pointer manager, uint16_t slaveIdx, uint16_t address, uint32_t data)
{
